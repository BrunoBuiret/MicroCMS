...
$app->match('/article/{id}', function($id, Request $request) use($app) {
    ...
    if($app['security']->isGranted('IS_AUTHENTICATED_FULLY'))
    {
        ...
        if($commentForm->isSubmitted() && $commentForm->isValid())
        {
            ...
            $app['session']->getFlashBag()->add('success', 'Your comment was sucessdully added.');
        }
...
$app->match('/admin/user/add', function(Request $request) use($app) {
    ...
    if ($userForm->isSubmitted() && $userForm->isValid())
    {
        ...
        $app['session']->getFlashBag()->add('success', sprintf('User %s was successfully created.', $user->getUsername()));
...
$app->match('/admin/user/{id}/edit', function($id, Request $request) use($app) {
    ...
    if ($userForm->isSubmitted() && $userForm->isValid())
    {
        ...
        $user->setPassword($password); 
        $app['dao.user']->save($user);
        $app['session']->getFlashBag()->add('success', sprintf('User %s was succesfully updated.', $user->getUsername()));
